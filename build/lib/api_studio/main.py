...
def create_app() -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        """Start background tasks on startup and ensure they shut down cleanly."""
        logger.info("API Studio starting up...")
    ...
    app = FastAPI(title="Clockify API Studio", version="0.1.0", lifespan=lifespan)
    ...
    async def manifest() -> JSONResponse:
        ...
        manifest_path = Path(__file__).resolve().parents[1] / "manifest.api-studio.json"
...
async def periodic_cleanup_task(stop_event: asyncio.Event):
    ...
    while not stop_event.is_set():
        try:
            # Wait 24 hours between cleanup runs unless shutdown is requested
            try:
            ...
            async with async_session_maker() as session:
                ...
                for table, result in results.items():
                    if result["status"] == "success":
                        ...
                        logger.error(
                            f"Failed to clean up {table}: {result.get('error')}"
                        )
        except Exception as e:
